...
def eval_psnr(loader, model, data_norm=None, eval_type=None, eval_bsize=None,
    ...
    for batch in pbar:
        ...
        val_loss.add(vloss.item())

        if verbose:
            pbar.set_description('val {} {:.4f}'.format("loss", val_loss.item()))
    ...
    return (val_loss.item(), (val_metric1.item(), val_metric2.item(), val_metric3.item(), val_metric4.item()), (metric1, metric2, metric3, metric4))
...
if __name__ == '__main__':
    ...
    (val_loss, (metric1, metric2, metric3, metric4), (metric1_fn, metric2_fn, metric3_fn, metric4_fn)) = eval_psnr(loader, model,
                                                   data_norm=config.get('data_norm'),
                                                   eval_type=config.get('eval_type'),
                                                   eval_bsize=config.get('eval_bsize'),
                                                   verbose=True)
    
    print('{}: {:.4f}'.format("val_loss", val_loss))
    print('{}: {:.4f}'.format(metric1_fn, metric1))
    print('{}: {:.4f}'.format(metric2_fn, metric2))
    print('{}: {:.4f}'.format(metric3_fn, metric3))
    print('{}: {:.4f}'.format(metric4_fn, metric4))
