...
# from .gdip_dfft import GatedDIP

from .gdip_s import GatedDIP
...
class VisionTransformerDVPSerial(nn.Module):
    ...
    def __init__(self, model_name='vit_large_patch16_384', img_size=384, patch_size=16, in_chans=3, embed_dim=1024, depth=24,
        ...
        # super(VisionTransformerEVP, self).__init__(**kwargs)
        super(VisionTransformerDVPSerial, self).__init__()
        ...
        # self.gdip_vpt = nn.Parameter(torch.zeros(
        #     1,self.depth,self.num_patches, self.embed_dim))
        ...
        self.tuning_stage = kwargs['tuning_stage']
        ...
        self.prompt_generator = PromptGenerator(self.scale_factor, self.prompt_type, self.embed_dim,
                                                ...
                                                img_size, patch_size,self.num_patches)
    ...
    def forward(self, x):
        ...
        handcrafted_feature,x_temp,emb,_ = self.prompt_generator.init_handcrafted(inp)
        prompt = self.prompt_generator.get_prompt(handcrafted_feature, embedding_feature,0)
        ...
        for i, blk in enumerate(self.blocks):
            if i != 0:
                x_t= self.patch_embed(x_temp)
                embedding_feature = self.prompt_generator.init_embeddings(x_t)
                handcrafted_feature,x_temp,emb,_ = self.prompt_generator.init_handcrafted(x_temp)
                prompt = self.prompt_generator.get_prompt(handcrafted_feature, embedding_feature,i)
            x = torch.cat((
                x[:, :1, :],
                prompt +x[:, 1:, :]+emb.unsqueeze(1),
            ), dim=1)
            x = blk(x)
...
class PromptGenerator(nn.Module):
    def __init__(self, scale_factor, prompt_type, embed_dim, tuning_stage, depth, input_type,
                 freq_nums, handcrafted_tune, embedding_tune, adaptor, img_size, patch_size,num_patch_emb):
        """
        Args:
        """
        ...
        self.gdip = GatedDIP(encoder_output_dim=self.embed_dim)
        self.num_patches = num_patch_emb
        ...
        self.prompt_generator = PatchEmbed(img_size=img_size,
                                                   patch_size=patch_size, in_chans=3,
                                                   embed_dim=self.embed_dim//self.scale_factor)
        # self.gdip_vpt = nn.Parameter(torch.zeros(
        #     1,self.depth,self.num_patches, self.embed_dim))
        # self.gdip_proj = nn.ModuleList([nn.Linear(self.embed_dim*2,self.embed_dim) for i in range(self.depth)])

        self.apply(self._init_weights)
        # trunc_normal_(self.gdip_vpt, std=.02)
    ...
    def init_handcrafted(self, x):
        ...
        x,emb,g = self.gdip(x)
        return self.prompt_generator(x),x,emb,g
    ...
    def get_prompt(self, handcrafted_feature, embedding_feature,depth):
        N, C, H, W = handcrafted_feature.shape
        handcrafted_feature = handcrafted_feature.view(N, C, H*W).permute(0, 2, 1)
        lightweight_mlp = getattr(self, 'lightweight_mlp_{}'.format(str(depth)))
        # prompt = proj_prompt(prompt)
        prompt = lightweight_mlp(handcrafted_feature + embedding_feature)
