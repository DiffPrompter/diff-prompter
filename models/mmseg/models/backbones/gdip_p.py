...
import torch
import torchvision 
from .gdip_vision_p import VisionEncoder
...
class GatedDIP(torch.nn.Module):
    ...
    def forward(self, x : torch.Tensor):
        ...
        gdip_dict = {
            "final": x,
            "wd": min_max_norm(wb_out),
            "gamma": min_max_norm(gamma_out),
            "identity": min_max_norm(identity_out),
            "fog": min_max_norm(fog_out),
            "sharpning": min_max_norm(sharpning_out),
            "contrast": min_max_norm(contrast_out),
            "tone": min_max_norm(tone_out),
            "fft": min_max_norm(fft_out),
        }

        return x,emb,gate,gdip_dict
