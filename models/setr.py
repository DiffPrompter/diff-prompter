...
from mmseg.models import backbones  ##DBM
from mmseg.models.builder import BACKBONES, SEGMENTORS, HEADS ##DBM
import os
import logging
...
class SETR(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None):
        ...
        elif 'dvp_p' in encoder_mode['name']:
            backbone = dict(type='VisionTransformerDVPParallel', img_size=inp_size, pos_embed_interp=True, drop_rate=0.,
                  model_name='vit_base_patch16_384', embed_dim=768,depth=12, num_heads=12,
                  scale_factor=encoder_mode['scale_factor'], input_type=encoder_mode['input_type'],
                  freq_nums=encoder_mode['freq_nums'], prompt_type=encoder_mode['prompt_type'],
                  tuning_stage=encoder_mode['tuning_stage'], handcrafted_tune=encoder_mode['handcrafted_tune'],
                  embedding_tune=encoder_mode['embedding_tune'], adaptor=encoder_mode['adaptor'], )
        elif 'dvp_s' in encoder_mode['name']:
            backbone = dict(type='VisionTransformerDVPSerial', img_size=inp_size, pos_embed_interp=True, drop_rate=0.,
                  model_name='vit_base_patch16_384', embed_dim=768,depth=12, num_heads=12,
                  scale_factor=encoder_mode['scale_factor'], input_type=encoder_mode['input_type'],
                  freq_nums=encoder_mode['freq_nums'], prompt_type=encoder_mode['prompt_type'],
                  tuning_stage=encoder_mode['tuning_stage'], handcrafted_tune=encoder_mode['handcrafted_tune'],
    ...
    def get_loss(self, pred_mask, gt_mask):
        loss_G = self.criterionBCE(pred_mask, gt_mask)
        if self.loss_mode == 'iou':
            loss_G += _iou_loss(pred_mask, gt_mask)
        return loss_G
